...
step_ssh() {
    ...
    # Home IP Auto-Allow via myfritz
    CLOWNFISCHSERVER_HOME_IP_DOMAIN=""
    echo ""
    echo -e "  ${BOLD}${CYAN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo -e "  ${BOLD}  myfritz Home-IP Automatik${NC}"
    echo -e "  ${CYAN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo -e "  SSH von Zuhause immer offen – ohne Port-Knocking."
    echo -e "  ${CYAN}  Clownfischserver aktualisiert deine Home-IP alle 5 Min automatisch.${NC}"
    echo ""

    if ask_yn "myfritz Home-IP permanent erlauben?"; then
        echo ""
        echo -e "  ${YELLOW}?${NC} ${BOLD}Deine myfritz Adresse:${NC}"
        echo -e "  ${CYAN}  Format: vorname.myfritz.net${NC}"
        read -r -p "    → " CLOWNFISCHSERVER_HOME_IP_DOMAIN

        if [[ -n "$CLOWNFISCHSERVER_HOME_IP_DOMAIN" ]]; then
            ok "myfritz Adresse gespeichert: $CLOWNFISCHSERVER_HOME_IP_DOMAIN"
            info "⏱ Clownfischserver-HomeIP Updater wird alle 5 Min gestartet"
        else
            warn "Leere Adresse – myfritz Automatik deaktiviert"
            CLOWNFISCHSERVER_HOME_IP_DOMAIN=""
        fi
    else
        info "myfritz Automatik deaktiviert"
    fi
...
step_ollama() {
    ...
    if [[ "$RAM_MB" -ge 24000 ]]; then
        CTX_DEFAULT=32768
        echo -e "  ${CYAN}  [1]  4096  – minimal, schnell${NC}"
        echo -e "  ${CYAN}  [2]  8192  – gut fuer /shell${NC}"
        echo -e "  ${CYAN}  [3] 16384  – ideal fuer /code${NC}"
        echo -e "  ${BOLD}  [4] 32768  – maximale Code-Qualitaet ← empfohlen${NC}"
    elif [[ "$RAM_MB" -ge 16000 ]]; then
        CTX_DEFAULT=16384
        echo -e "  ${CYAN}  [1]  4096  – minimal, schnell${NC}"
        echo -e "  ${CYAN}  [2]  8192  – gut fuer /shell${NC}"
        echo -e "  ${BOLD}  [3] 16384  – ideal fuer /code ← empfohlen${NC}"
        echo -e "  ${CYAN}  [4] 32768  – maximale Qualitaet (braucht viel RAM)${NC}"
    elif [[ "$RAM_MB" -ge 12000 ]]; then
        CTX_DEFAULT=8192
        echo -e "  ${CYAN}  [1]  4096  – minimal${NC}"
        echo -e "  ${BOLD}  [2]  8192  – guter Kompromiss ← empfohlen${NC}"
        echo -e "  ${CYAN}  [3] 16384  – groessere Scripts${NC}"
        echo -e "  ${CYAN}  [4] 32768  – nur wenn genuegend RAM${NC}"
    else
        CTX_DEFAULT=4096
        echo -e "  ${BOLD}  [1]  4096  – passend fuer wenig RAM ← empfohlen${NC}"
        echo -e "  ${CYAN}  [2]  8192  – wenn es reicht${NC}"
        echo -e "  ${CYAN}  [3] 16384  – wahrscheinlich zu viel${NC}"
        echo -e "  ${CYAN}  [4] 32768  – nicht empfohlen${NC}"
    fi
...
step_telegram() {
    ...
    if ask_yn "OpenRouter als Fallback aktivieren?"; then
        ...
        read -r -p "    → " OPENROUTER_API_KEY
        
        if [[ -z "$OPENROUTER_API_KEY" ]]; then
            warn "Leerer Key – OpenRouter deaktiviert"
            OPENROUTER_API_KEY=""
        else
            ok "OpenRouter aktiviert"
        fi
    else
        info "OpenRouter deaktiviert – Bot nutzt nur lokal Ollama"
    fi
...
CLOWNFISCHSERVER_HOME_IP_DOMAIN=$CLOWNFISCHSERVER_HOME_IP_DOMAIN
ENVEOF
...
BOOTEOF
    ...
    # Clownfischserver HomeIP Updater (wenn aktiviert)
    if [[ -n "$CLOWNFISCHSERVER_HOME_IP_DOMAIN" ]]; then
        mkdir -p "$INSTALL_DIR/scripts"
        cp "$SCRIPT_DIR/scripts/clownfischserver-homeip-updater.sh" "$INSTALL_DIR/scripts/" 2>/dev/null || \
            warn "homeip-updater.sh nicht gefunden – manuell nachträglich einrichten"

        if [ -f "$INSTALL_DIR/scripts/clownfischserver-homeip-updater.sh" ]; then
            chmod +x "$INSTALL_DIR/scripts/clownfischserver-homeip-updater.sh"
            chown root:root "$INSTALL_DIR/scripts/clownfischserver-homeip-updater.sh"
...
print_summary() {
    ...
    if [[ -n "$CLOWNFISCHSERVER_HOME_IP_DOMAIN" ]]; then
        echo -e "  ${BOLD}${GREEN}HomeIP Auto:${NC}  $CLOWNFISCHSERVER_HOME_IP_DOMAIN (alle 5 Min aktualisiert)"
    else
        echo -e "  ${BOLD}HomeIP Auto:${NC}  nicht aktiviert"
    fi
    ...
    if [[ -n "$CLOWNFISCHSERVER_HOME_IP_DOMAIN" ]]; then
        echo -e "  ${GREEN}${BOLD}🐠 Clownfischserver-HomeIP Status:${NC}"
        echo -e "  ${GREEN}  Cron-Job aktiv: /var/log/clownfischserver-homeip.log${NC}"
        echo ""
    fi
...
"
    ...
    # Update systemd services – patch hardcoded user with actual MGMT_USER
