...
CLOWNFISCHSERVER_HOME_IP_DOMAIN=vorname.myfritz.net  # Optional: myfritz für Auto-SSH-Freigabe
```

### Clownfischserver HomeIP Auto (neu in v0.5.0)

**Feature:** Automatische SSH-Freigabe für deine Home-IP alle 5 Minuten

- **myfritz.net:** DynDNS-Service (AVM Fritz.Box) – kostenlos
- **Cron-Job:** Läuft als `root` alle 5 Min
- **Workflow:**
  1. DNS-Auflösung: `vorname.myfritz.net` → IP-Adresse
  2. Vergleich: Alte IP vs. neue IP
  3. Bei Änderung: UFW-Regel aktualisieren
  4. Logging: `/var/log/clownfischserver-homeip.log`

**Voraussetzungen:**
- myfritz-Account bei https://myfritz.net/ (kostenlos)
- `dig` oder `nslookup` für DNS-Auflösung (meist bereits installiert)

**Beispiel:**
```bash
# Installation: wird vom install.sh gefragt
...
# Dann läuft:
*/5 * * * * /opt/clownfischserver/scripts/clownfischserver-homeip-updater.sh >> /var/log/clownfischserver-homeip.log 2>&1
```
